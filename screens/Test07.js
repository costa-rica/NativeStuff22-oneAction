...
import SwipePadFourAndFiveOptions from "./components/SwipePadFourAndFiveOptions";
...
export default function Test07({ navigation }) {
  ...
  const defaultColors = {
    ...
    rightright: "rgba(125, 150, 100, 0.25)",
    bottombottom: "rgba(150, 100, 125, 0.25)",
  ...
  const handleSwipeColorChange = (direction, outerDirection = false) => {
    ...
    const brightColors = {
      toptopleft: "rgba(175, 255, 255, 1)",
      toptopright: "green",
      rightright: "rgba(255, 255, 175, 1)",
      bottombottom: "rgba(255, 175, 255, 1)",
  ...
  const gestureSwipeOnChange = Gesture.Pan().onChange((event) => {
    ...
    if (inInnerCircle) {
      ...
      if (
        ...
        if (!inMiddleCircle) {
          handleSwipeColorChange("right", "rightright");
          setCurrentActionType("rightright");
        }
      } else if (
        relativeToPadCenterY > 0 &&
        Math.abs(relativeToPadCenterX) < Math.abs(relativeToPadCenterY)
